...
if __name__ == "__main__":
    ...
    secret_name = os.getenv("SECRET_NAME", "yahoo_fin_api")  # AWS Secrets Manager secret name
    bucket_name = os.getenv("S3_BUCKET_NAME", "yahoo-finance-stock")  # S3 bucket name
    symbols = os.getenv("STOCK_SYMBOLS", "AAPL,MSFT,GOOGL").split(",")  # Stock symbols
    try:
        # Fetch API key from AWS Secrets Manager
        secrets = fetch_secret(secret_name)
        print(secrets)
        api_key = secrets['yahoo_f_api_key']
        ...
        write_to_s3(bucket_name, stock_data, "raw_data")
